...
#   - How to choose split the data to train 5 separate classifiers? Consider class imbalance DONE
#         - Implement Random Sampling (No Duplicates) From Both Classes for the data used in each classifier (4k each)
#   - What is the criteria to choose which class an instance belongs to? (i.e. confidence > 0.75) DONE
#         - Current criteria: 0.75 
#   - How do we average the result? DONE
...
# - Compared bootstrap sampling (replacement) to just randomly selecting it from the pool of samples 
  ...
  # - Bootstrap sampling doesn't affect the performance compared to if it is randomly sampled without replacement 
...
#   x_training.extend(resample(incomeDataGreat, n_samples=4000, replace=True))
#   x_training.extend(resample(incomeDataLess, n_samples=4000, replace=True))
